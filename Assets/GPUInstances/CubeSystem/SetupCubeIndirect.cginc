...
StructuredBuffer<CubePropertiesData> _Props;
...
void setup(){
    ...
    CubePropertiesData data                = _Props[unity_InstanceID];                           //retreive the data per instance from the structuredBuffer. unity_InstanceID is the index of each instance
                       unity_ObjectToWorld = data.TRS;//mul(data.transform, mul(data.rotate, data.scale));  //redefine the Object to World matrix based on the matrices bounds to the shader via the StructuredBuffer
